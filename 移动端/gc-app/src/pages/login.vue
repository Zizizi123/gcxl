<template>
    <div class="login">
        ...
        <div class="forgets" >
               <a
        href="https://github.com/login/oauth/authorize?client_id=a48f1774b2f9ce18125e" style="text-
        align:center"
        >Github</a>
          
            <router-link to="/quicker_login">快速登录</router-link>

            <router-link to="/resetPass">忘记密码?</router-link>
        </div>
       <div class="forgets" >
                  <router-link to="/regist">注册账号</router-link>
            <router-link to="/quicker_regist">快速注册</router-link>
        </div>
    </div>
</template>
...
export default {
    ...
    if (code1) {
      ...
      this.$ajax({
        method: 'get',
        url: '/api/github/result',
        params: {
          code: code1,
          role: this.info.role
        }
      }).then(response => {
        console.log("resssss------1", response)
